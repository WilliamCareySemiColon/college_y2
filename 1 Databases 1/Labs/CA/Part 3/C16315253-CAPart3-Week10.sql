...
-- 5 marks
--updating the table to add markup coloumn
ALTER TABLE Product
...
--setting the value of the msrkupin each version off the table
UPDATE Product
SET Product.markup = (Product.retailPrice - Product.costPrice)/100;
--3. Add a constraint to the markup column so that it cannot contain null
-- 5 marks
--making sure it cannot contain null values
ALTER TABLE Product
...
--setting all null drugondrug to be 0
UPDATE Product
SET Product.drugnondrug = 0
WHERE Product.drugnondrug = NULL;

--5.
--Remove the column retailprice from the product table.
-- 5 marks
--removing retailprice from column
ALTER TABLE Product
...
-- 10 marks
--turning drugondrug into a boolean variable
ALTER TABLE Product
ADD CONSTRAINT drugnondrug_ck 
CHECK (drugnondrug = 0 OR drugnondrug = 1);
...
-- 10 marks
--changing the dosage value of Calpol to be 50
UPDATE PrescriptionItem
SET PrescriptionItem.preDosage = 50
where PrescriptionItem.stockCode = (
SELECT stockCode FROM Product
WHERE stockDescription = 'Calpol'
);
--8.
...
-- 15 marks
--removing Dannys perscription and all thier info
DELETE FROM PrescriptionItem
WHERE prescriptionID =( 
SELECT prescriptionID FROM Prescription
WHERE custID = (
SELECT custID FROM Customer WHERE custName = 'Danny')
);

-- Change all nondrugsales of Umberella to be for product Lynx. You need to use sub-queries
--10 marks
--changing unbrella details to lynx
UPDATE NonDrugSale
SET stockCode = 10
WHERE stockCode = 
    (SELECT stockCode FROM Product 
    WHERE stockDescription = 'Umbrella')
;    
...
-- 15 marks
--deleting the sale associated with lynx
DELETE FROM NonDrugSale
WHERE stockCode = 
(SELECT stockCode FROM Product
WHERE stockDescription = 'Lynx');
--deleting lynx itself
DELETE FROM Product
WHERE stockDescription = 'Lynx';
--11
...
--You should ensure case sensitivity is not an issue.
-- 15 marks
--adding coloumn notes to database
ALTER TABLE Prescription
ADD Notes VARCHAR(100);

--setting the values of notes based upon the doctor associated within
UPDATE Prescription
SET Notes = 
    (SELECT CASE docName 
    WHEN 'Lindsey' THEN 'Verify instructions'
    WHEN 'Jack' THEN 'Verify instructions'
    WHEN  'Michael' THEN 'Check with customers'
    ELSE 'Discuss with George'
    END AS INSTRUCTIONS_FROM_DOC
    FROM Doctor
    WHERE (Prescription.docID = Doctor.docID)
);
--making sure it worked
SELECT Notes From Prescription;
COMMIT;
