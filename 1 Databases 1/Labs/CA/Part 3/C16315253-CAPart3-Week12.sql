...
--taking those who sold items or perscribed the items

--Select items from PrescriptionItem Table from two different tables 
--and combining the result using union 

--those who perscribed the item
 SELECT stockDescription "Product name", staffName "Staff Member"
 FROM PrescriptionItem
 --joining the tables using their unique idtifier to have access to coloumns which we need
 INNER JOIN Product USING (stockCode) 
 INNER JOIN Staff USING (staffID)
 --combing the result of the two tables who sold or perscribed
 UNION
 --those who sold the item
 SELECT stockDescription "Product name", staffName "Staff Member" 
 FROM NonDrugSale
 INNER JOIN Staff USING (staffID)
 INNER JOIN Product USING (stockCode);
...
--taking those who sold items and perscribed the items
--Select items from PrescriptionItem Table from two different tables 
--and forced to find a common name using intersect
 SELECT stockDescription "Product name", staffName "Staff Member"
 FROM PrescriptionItem
 --joining the tables using their unique idtifier to have access to coloumns which we need
 INNER JOIN Product USING (stockCode) 
 INNER JOIN Staff USING (staffID)
 --forcing the table to find what is common to both
 INTERSECT
  --those who sold the item
 SELECT stockDescription "Product name", staffName "Staff Member" 
 FROM NonDrugSale
...
--creating two tables so we can have access to the information in a quicker rate
CREATE VIEW CustomerDetails AS
SELECT custName, prescriptionID FROM Prescription
INNER JOIN Customer USING (custID);

CREATE VIEW CustomerDet AS
SELECT custName, staffName, staffID, prescriptionID FROM Prescription
INNER JOIN Customer USING (custID)
INNER JOIN Staff USING (staffID);

--SELECTING the data from the two tables where the 
--we use the table to combine the result
SELECT staffName "Staff Name", stockDescription "Product Name", 
custName "Customer Name" FROM PrescriptionItem
--joinig the tables and view which allow us to access the information
INNER JOIN CustomerDetails USING (prescriptionID)
INNER JOIN Product USING (stockCode)
INNER JOIN Staff USING (staffID)
--setting the restrictions so we get the results we need
WHERE staffName LIKE 'B%'
OR stockDescription LIKE 'B%'
OR custName LIKE 'B%'
--repeating the process in another query
UNION
SELECT staffName, stockDescription, custName FROM NonDrugSale
INNER JOIN CustomerDet USING (staffID)
INNER JOIN Product USING (stockCode)
WHERE staffName LIKE 'B%'
OR stockDescription LIKE 'B%'
OR custName LIKE 'B%';
...
-- 20 marks
--selecting all the doctors names from doctor
SELECT docName "DOCTOR Name" FROM Doctor
--forcing it to be similar to the nwxt query
INTERSECT
--checking for doctors that have not perscribed on any items
SELECT docName FROM Doctor
LEFT OUTER JOIN Prescription ON (Prescription.docID = Doctor.docID)
...
--searching for doctors that have perscribed items
SELECT docName "DOCTOR Name" FROM Doctor
LEFT OUTER JOIN Prescription ON (Prescription.docID = Doctor.docID)
...
--5 marks
--selecting the doctors and the number of times they have perscirbed any items
SELECT docName "DOCTOR Name",COUNT(prescriptionID) "Number of items perscribed"
FROM Prescription
INNER JOIN Doctor USING (docID)
GROUP BY docName, docID;
...
LEFT OUTER JOIN Prescription ON (Prescription.docID = Doctor.docID)
WHERE prescriptionID IS NULL
GROUP BY docName, prescriptionID;
...
--10 marks
--joining part a and part b and create it as a view
CREATE VIEW DoctorLeagueTable AS
SELECT docName "DOCTOR Name",COUNT(docID) "Number of items perscribed"
FROM Prescription
INNER JOIN Doctor USING (docID)
GROUP BY docName, docID
UNION
SELECT docName, COUNT(prescriptionID) FROM Doctor
LEFT OUTER JOIN Prescription ON (Prescription.docID = Doctor.docID)
WHERE prescriptionID IS NULL
GROUP BY docName, prescriptionID;
--select everything from views
