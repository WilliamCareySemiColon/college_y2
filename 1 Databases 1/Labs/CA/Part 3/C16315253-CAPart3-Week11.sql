...
-- 10 marks 
--selecting all drugs that are not perscribed
...
--Displaying the number of times he item has been persribed except those not perscribed
SELECT COUNT(stockCode) "Number of times perscribed",
stockDescription "Name of Item" FROM Product
LEFT OUTER JOIN PrescriptionItem USING (stockCode)
...
--this is the same as question 1 but i will oblidged
SELECT stockDescription FROM Product
LEFT OUTER JOIN PrescriptionItem USING (stockCode)
WHERE Drugnondrug = 1 AND prescriptionID IS NULL;
...
--selecting the perscribtion for every customer but display none if the customer did not get any perscription
SELECT custName "Customer Name",
nvl(prescriptionID,0) "PrescriptionID" 
FROM Customer
LEFT OUTER JOIN Prescription USING (custID);
...
--displaying all the customers and their assocaitive doctor and perscription
SELECT custName "Customer Name", nvl(docName,'NA') "Doctor's Name",
nvl(to_char(prescriptionID),'NA') "Prescription ID" FROM Prescription
...
--dropping pervious version of view
DROP VIEW CustomerPrescription;
--selecting all information about the customer and their respective doctors/perscription
--and dumping it into a view called CustomerPrescription
CREATE VIEW CustomerPrescription AS 
SELECT custName "Customer Name", docName "Doctor's Name", 
prescriptionID "Prescription ID",custAddress "Customer Address", 
custPhone "Customer Phone Number" FROM Prescription
INNER JOIN Doctor USING (docID)
RIGHT OUTER JOIN Customer USING (custID)
WHERE docName IS NOT NULL AND prescriptionID IS NOT NULL;
...
-- 15 marks
SELECT custName "Customer Name", nvl(docName,'NA') "Doctor's Name",
nvl(to_char(prescriptionID),'NA') "Prescription ID" FROM Customer
LEFT OUTER JOIN 
(
    SELECT docName, prescriptionID, custID FROM Prescription
    INNER JOIN Doctor USING (docID)
)Prescription_Allocator ON (Customer.custID = Prescription_Allocator.custID);
...
--creatinf the view that contains the details of the perscription, 
--which is their name, id and staff member who dispesened it
CREATE VIEW PrescriptionDetail AS
SELECT stockDescription "StockDescription", prescriptionID "PrescriptionID", 
staffName "DispensedBy" FROM PrescriptionItem
